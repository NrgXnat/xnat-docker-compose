...
- **webapps** specifies the Maven coordinates for the XNAT war file. The retrieved file is written to the folder _base_`/webapps`, which
  ...
  form _groupId_:_artifactId_:_version_, e.g. for XNAT 1.8.3, this would be `org.nrg.xnat.web:xnat-web:1.8.3`.
...
context path for the application. For example, XNAT 1.8.3 is named `xnat-web-1.8.3.war`. This would result in XNAT being available at
the URL http://localhost/xnat-web-1.8.3, which is not exactly concise and would also change if you upgraded to XNAT 1.8.4. Instead you
...
```
$ ./gradlew -PenvFile=xnat.env -Pmanifest=manifest-1.8.3.json fullStackComposeBuild
```
...
```
{
    "version": "1.8.3",
    "base": "xnat-data",
    "webapps": "org.nrg.xnat.web:xnat-web:1.8.3 -> ROOT.war",
    "plugins": [
...
This tells the build to get XNAT 1.8.3 and save it as `xnat-data/webapps/ROOT.war`. It also tells the build to download the [OHIF
...
XNAT 1.8.3, as well as a number of plugins to support datasets, container management, and machine learning operations including training
